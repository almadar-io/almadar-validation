...
const RUST_ERROR_FILE = join(__dirname, "../../orbital-rust/crates/orbital-compiler/src/phases/validation/error.rs");
const OUTPUT_FILE = join(__dirname, "error-codes.ts");
function parseRustErrorCodes(rustCode) {
    const codes = [];
    ...
    const categoryRegex = /ErrorCode::(\w+)(?:\s*\|[^=]*)*\s*=>\s*"(\w+)"/g;
    const categorySection = rustCode.match(/fn category\(&self\)[^{]*\{([\s\S]*?)^\s{4}\}/m);
    if (categorySection) {
...
function inferCategory(code) {
    if (code.startsWith("ORB_E_"))
        return "entity";
    if (code.startsWith("ORB_T_EVT_"))
        return "trait_event";
    if (code.startsWith("ORB_T_"))
        return "trait";
    if (code.startsWith("ORB_P_"))
        return "page";
    if (code.startsWith("ORB_SLOT_"))
        return "slot";
    if (code.startsWith("ORB_RUI_"))
        return "render_ui";
    if (code.startsWith("ORB_EFF_"))
        return "effect";
    if (code.startsWith("SEXPR_"))
        return "sexpr";
    if (code.startsWith("ORB_SVC_"))
        return "service";
    if (code.startsWith("ORB_X_"))
        return "cross";
    if (code.startsWith("ORB_BINDING_"))
        return "binding";
    if (code.startsWith("CIRCUIT_"))
        return "circuit";
    if (code.startsWith("ORB_UI_ICON_"))
        return "icon";
    if (code.startsWith("ORB_QUERY_"))
        return "query";
    if (code.startsWith("ORB_S_"))
        return "schema";
    return "unknown";
}
